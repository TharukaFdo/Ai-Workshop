...
- Identify Inventory Request and the main workflow.
...
Create or update REQUIREMENTS.md for Inventory Request System.

Case details:
- Roles: Staff member, Storekeeper
- Staff member actions: submit inventory requests, view own request status, update own pending request details, filter own requests
- Storekeeper actions: review all requests, approve or reject requests, add storekeeper notes, mark approved items as issued, filter requests
...
- Initial status values before Stage 11: pending, approved, rejected, issued
- Main feature: inventory request submission, approval/rejection and issue workflow
...
Important fields:
itemName, quantity, reason, requestedDate, requesterName, status, storekeeperNote, issuedQuantity, issuedAt, createdAt, updatedAt
...
```text
Implement the frontend workflow skeleton for Inventory Request System.
...
Main workflow:
inventory request submission, approval/rejection and issue workflow

Secondary feature:
filter requests by item name, requester or status

Instructions:
- Create role-aware screens for Staff member and Storekeeper.
- Create forms for the important Inventory Request fields.
...
```text
Implement the core feature end-to-end for Inventory Request System.
...
```text
Add workshop-suitable authentication and backend authorization for Inventory Request System.
...
```text
Implement the secondary feature for Inventory Request System.
...
Instructions:
- Keep the feature small and directly connected to Inventory Request.
- Add only the backend route/query changes needed.
- Add only the frontend UI changes needed.
- Ensure the feature respects Staff member and Storekeeper permissions.
