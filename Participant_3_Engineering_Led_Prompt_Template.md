...
Participant 3 is the strongest version of the workshop process. These prompts are designed to produce the best possible workshop project from the selected case: clear requirements, controlled scope, maintainable React, Express, Node.js, and MySQL structure, local MySQL data handling, backend access control, testing, security review, refactoring, change handling, and final explanation.
...
- Keep the stack fixed as React, Node.js/Express, and local MySQL.
- Keep Express as the API layer between React and MySQL.
- Do not allow React to connect directly to MySQL or contain MySQL credentials.
- Keep the prompts detailed, but make them directly usable in Antigravity IDE's AI chat/agent.
...
```text
Inspect the current codebase, then create or update the React, Express, Node.js, and MySQL project backbone for [CASE_TITLE].
...
- Prepare a placeholder for DB_HOST, DB_PORT, DB_USER, DB_PASSWORD, and DB_NAME in backend `.env.example`.
...
- Include a database-backed prototype login table, for example `app_users`, with role and ownership/identity fields for [ROLE_1] and [ROLE_2].
...
- Add example seed data, including demo users for [ROLE_1] and [ROLE_2].
...
- Ensure [ROLE_2] can access only their assigned/owned records where the case requires it.
...
Context:
This is [CASE_TITLE], a React, Express, Node.js, and MySQL app using React, Node.js/Express, and local MySQL.
