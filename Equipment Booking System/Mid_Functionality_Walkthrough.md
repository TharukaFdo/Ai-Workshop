# Equipment Booking System: Mid Functionality Walkthrough
...
- Expected early status values: pending, approved, rejected
...
| Staff member allowed actions | As Staff member, check: request equipment bookings, view own bookings, update own pending booking details, filter own bookings, view booking status. | |
| Lab assistant allowed actions | As Lab assistant, check: view all booking requests, approve or reject bookings, add assistant comments, filter bookings. | |
| Protected action allowed | As the correct role, attempt: approve or reject bookings and add assistant comments. | |
| Protected action blocked | As the wrong role, attempt: approve or reject bookings and add assistant comments. It should be blocked by the backend, not only hidden in the UI. | |
| Own-record restriction | Try to access or modify another user role/member/owner record where relevant. It should be blocked. | |
| Main workflow | Complete the workflow: equipment booking request create, view, update and approve/reject workflow. | |
| Secondary feature | Use the secondary feature: filter bookings by equipment, date or status. | |
| Validation | Try invalid or missing data based on: equipment name, requested user, booking date, start time, end time and purpose are required; status must use valid values; booking date/time must be valid; end time must be after start time. | |
