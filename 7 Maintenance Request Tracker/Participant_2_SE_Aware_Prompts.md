...
```text
Revise for Maintenance Request Tracker. Keep React, Express, Node.js, and MySQL, small scope, requester, technician, maintenance request, validation, backend role access, database-backed login, add or edit technician notes and close requests, filter requests by location, priority or status, automated checks, and fix the issue. Do only the current stage.
```
## Error Prompt
