# Participant 2: Internship Application Tracker Prompts
...
```text
Summarize Internship Application Tracker: roles student and coordinator, main entity internship application, main feature internship application submission, review and status update workflow, secondary feature filter applications by company name or application status, and out of scope.
```

### Stage 1: Requirements

```text
List requirements for Internship Application Tracker: main workflow internship application submission, review and status update workflow, create/view/update/status actions where appropriate, roles, validation, login, protected action add or edit coordinator comments and approve or reject applications, and basic tests.
```
...
```text
Design Supabase table for internship application using studentName, companyName, positionTitle, startDate, endDate, submittedDate, status, coordinatorComment, createdAt, updatedAt, initial status values submitted, underReview, approved, rejected, required fields, and role fields.
```
...
```text
Implement internship application submission, review and status update workflow for internship application with React pages, Express routes, Supabase queries, validation, and errors.
```

### Stage 6: Authentication And Authorization

```text
Add login and authorization for student and coordinator. Protect add or edit coordinator comments and approve or reject applications in backend and UI.
```
...
```text
Create basic tests or manual checks for the main workflow, validation, login, roles, add or edit coordinator comments and approve or reject applications, and filter applications by company name or application status.
```
...
```text
Apply change request: coordinators can request changes, and students can edit and resubmit only applications with changes requested. Update data, API, UI, validation, roles, and tests without rewriting the app.
```
...
```text
Revise for Internship Application Tracker. Keep PERN, small scope, student, coordinator, internship application, validation, role access, add or edit coordinator comments and approve or reject applications, filter applications by company name or application status, and fix the issue.
```
