...
```text
Add database-backed login and authorization for student and coordinator. Protect add or edit coordinator comments and approve or reject applications in backend and UI. Use the users/login table from Stage 3. Do not use hard-coded frontend-only accounts.
```
...
```text
Revise for Internship Application Tracker. Keep PERN, small scope, student, coordinator, internship application, validation, backend role access, database-backed login, add or edit coordinator comments and approve or reject applications, filter applications by company name or application status, and fix the issue, automated checks, and do only the current stage.
```
## Error Prompt
