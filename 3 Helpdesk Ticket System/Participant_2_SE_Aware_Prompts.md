# Participant 2: Helpdesk Ticket System Prompts
...
```text
Summarize Helpdesk Ticket System: roles user and support agent, main entity ticket, main feature ticket creation, response, status update and closure workflow, secondary feature filter tickets by category, submitted user or status, and out of scope. Do not create project files yet.
```

### Stage 1: Requirements

```text
List requirements for Helpdesk Ticket System: main workflow ticket creation, response, status update and closure workflow, create/view/update/status actions where appropriate, roles, validation, login, protected action add or edit agent responses and close tickets, and basic automated tests. Do not create application code yet.
```
...
```text
Design MySQL table for ticket using title, description, category, submittedUser, status, agentResponse, createdAt, updatedAt, closedAt, initial status values open, inProgress, resolved, closed, required fields, and role fields.
...
```text
Create UI workflow for user and support agent: pages, forms, list, filters, role actions, loading, and errors.
```

### Stage 5: Main Feature

```text
Implement ticket creation, response, status update and closure workflow for ticket with React pages, Express routes, MySQL queries, validation, and errors.
```

### Stage 6: Authentication And Authorization

```text
Add database-backed login and authorization for user and support agent. Use the users/login table from Stage 3. Use a simple login-issued user identity or session token for later requests. Protected backend routes must look up/check the user's role and ownership from the database before allowing actions. Do not trust a role, user type, or owner name sent directly from the browser as proof of permission. Protect add or edit agent responses and close tickets in backend and UI. Do not use hard-coded frontend-only accounts.
```

### Stage 7: Secondary Feature

```text
Add filter tickets by category, submitted user or status for ticket. Update API, UI, and filters without adding unrelated features.
```
...
```text
Conduct a mid-project review for Helpdesk Ticket System.
...
- ticket category, status, and submitted user tracking
- agent response workflow and ticket closure
- user visibility limited to own tickets
...
- Whether add or edit agent responses and close tickets appears protected.
- Whether users appear limited to their own allowed records where relevant.
- Whether ticket creation, response, status update and closure workflow appears implemented.
- Whether filter tickets by category, submitted user or status appears implemented.
...
| Protected action |  |  |  |  |  |  |  |  |  |
| Secondary feature |  |  |  |  |  |  |  |  |  |
| Case-specific: ticket category, status, and submitted user tracking |  |  |  |  |  |  |  |  |  |
| Case-specific: agent response workflow and ticket closure |  |  |  |  |  |  |  |  |  |
| Case-specific: user visibility limited to own tickets |  |  |  |  |  |  |  |  |  |
| UI/manual usability |  |  |  |  |  |  |  |  |  |
| Security posture |  |  |  |  |  |  |  |  |  |
| Testing evidence |  |  |  |  |  |  |  |  |  |
| Maintainability |  |  |  |  |  |  |  |  |  |
...
```text
Prepare a final evidence-based review for Helpdesk Ticket System.
...
```text
Revise for Helpdesk Ticket System. Keep React, Express, Node.js, and MySQL, small scope, user, support agent, ticket, validation, backend role access, database-backed login, add or edit agent responses and close tickets, filter tickets by category, submitted user or status, automated checks, and fix the issue. Do only the current stage.
```
## Error Prompt
