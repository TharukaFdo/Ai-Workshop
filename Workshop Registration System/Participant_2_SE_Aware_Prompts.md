...
```text
Add database-backed login and authorization for participant and organizer. Protect mark attendance and edit organizer notes in backend and UI. Use the users/login table from Stage 3. Do not use hard-coded frontend-only accounts.
```
...
```text
Revise for Workshop Registration System. Keep PERN, small scope, participant, organizer, registration, validation, backend role access, database-backed login, mark attendance and edit organizer notes, filter registrations by workshop title, registration status or attendance status, and fix the issue, automated checks, and do only the current stage.
```
## Error Prompt
