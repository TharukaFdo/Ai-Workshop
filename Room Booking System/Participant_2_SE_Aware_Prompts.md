...
```text
Add database-backed login and authorization for staff member and coordinator. Protect approve or reject room bookings and edit coordinator notes in backend and UI. Use the users/login table from Stage 3. Do not use hard-coded frontend-only accounts.
```
...
```text
Revise for Room Booking System. Keep PERN, small scope, staff member, coordinator, room booking, validation, backend role access, database-backed login, approve or reject room bookings and edit coordinator notes, filter bookings by room, date or status, and fix the issue, automated checks, and do only the current stage.
```
## Error Prompt
