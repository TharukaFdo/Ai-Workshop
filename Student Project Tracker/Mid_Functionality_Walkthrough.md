# Student Project Tracker: Mid Functionality Walkthrough
...
| Student allowed actions | As Student, check: create project submissions, update own editable submissions, view own status, filter own submissions. | |
| Supervisor allowed actions | As Supervisor, check: view submitted projects, add feedback, update project status, filter projects. | |
| Protected action allowed | As the correct role, attempt: add or edit supervisor feedback and approve or reject projects. | |
| Protected action blocked | As the wrong role, attempt: add or edit supervisor feedback and approve or reject projects. It should be blocked by the backend, not only hidden in the UI. | |
| Own-record restriction | Try to access or modify another user role/member/owner record where relevant. It should be blocked. | |
| Main workflow | Complete the workflow: project submission, supervisor review, feedback and status update workflow. | |
| Secondary feature | Use the secondary feature: filter projects by supervisor, category or status. | |
| Validation | Try invalid or missing data based on: title, description, category, student name, supervisor name and submitted date are required; status must use valid values; submitted date must be valid. | |
