...
```text
Add database-backed login and authorization for student and supervisor. Protect add or edit supervisor feedback and approve or reject projects in backend and UI. Use the users/login table from Stage 3. Do not use hard-coded frontend-only accounts.
```
...
```text
Revise for Student Project Tracker. Keep PERN, small scope, student, supervisor, project submission, validation, backend role access, database-backed login, add or edit supervisor feedback and approve or reject projects, filter projects by supervisor, category or status, and fix the issue, automated checks, and do only the current stage.
```
## Error Prompt
