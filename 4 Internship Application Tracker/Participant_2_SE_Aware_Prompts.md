...
```text
Revise for Internship Application Tracker. Keep PERN, small scope, student, coordinator, internship application, validation, backend role access, database-backed login, add or edit coordinator comments and approve or reject applications, filter applications by company name or application status, automated checks, and fix the issue. Do only the current stage.
```
## Error Prompt
