...
```text
Revise for Helpdesk Ticket System. Keep PERN, small scope, user, support agent, ticket, validation, backend role access, database-backed login, add or edit agent responses and close tickets, filter tickets by category, submitted user or status, automated checks, and fix the issue. Do only the current stage.
```
## Error Prompt
