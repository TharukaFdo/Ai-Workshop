...
```text
Add database-backed login and authorization for staff member and storekeeper. Protect approve or reject requests, mark items issued, and edit storekeeper notes in backend and UI. Use the users/login table from Stage 3. Do not use hard-coded frontend-only accounts.
```
