...
```text
Add database-backed login and authorization for receptionist and doctor. Protect add/edit visit notes in backend and UI. Use the users/login table from Stage 3. Do not use hard-coded frontend-only accounts.
```
...
```text
Revise for Clinic Appointment System. Keep PERN, small scope, receptionist, doctor, appointment, validation, backend role access, database-backed login, visit note protection, filtering, and fix the issue, automated checks, and do only the current stage.
```
## Error Prompt
